...
// ─── GET /ai-chat/history ─────────────────────────────────────────────────────
// (POST /usage-log moved to ai-chat-usage-log.ts — a separate, unauthenticated-
// by-user-session router, since it's an internal service-to-service call, not
// a user action. Mounting it on THIS router would inherit requireAivitaAuth
// above, which is exactly the trust model it must NOT use — see that file.)
