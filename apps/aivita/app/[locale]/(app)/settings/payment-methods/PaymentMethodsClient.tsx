...
function AddCardModal({ onClose, onAdded }: { onClose: () => void; onAdded: () => void }) {
  ...
  async function handleCardSubmit() {
    ...
    try {
      ...
      if (data.cardToken) { setCardToken(data.cardToken); setPhone(data.phone ?? ''); setStep('sms'); }
      else setError('Не удалось добавить карту');
  ...
  async function handleSmsVerify() {
    ...
    try {
      ...
      onAdded(); onClose();
    } catch { setError('Ошибка верификации'); }
    setLoading(false);
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-[480px] rounded-t-2xl bg-white p-6 pb-10" onClick={e => e.stopPropagation()}>
        ...
        {step === 'card' && (
          <>
            <h3 className="text-[16px] font-bold text-[#2a2540] mb-1">Карта {PROVIDER_STYLES[provider]?.label}</h3>
            ...
            <div className="space-y-3">
              ...
              <button onClick={handleCardSubmit} disabled={loading || cardNumber.replace(/\s/g, '').length < 16 || expiry.length < 4}
        ...
        {step === 'sms' && (
          <>
            ...
            <div className="space-y-3">
              <input value={smsCode} onChange={e => setSmsCode(e.target.value)} placeholder="000000" inputMode="numeric"
              ...
              {error && <p className="text-red-500 text-[12px]">{error}</p>}
              <button onClick={handleSmsVerify} disabled={loading || smsCode.length < 4}
                className="w-full rounded-xl bg-[#9c5e6c] text-white py-3.5 text-[14px] font-bold disabled:opacity-50 flex items-center justify-center">
...
export function PaymentMethodsClient() {
  ...
  return (
    <div className="max-w-[480px] mx-auto">
              ...
              <div key={method.id} className="rounded-xl border border-[#e8e4dc] bg-white p-4 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {method.isDefault && (
                      <span className="text-[10px] bg-[#9c5e6c]/10 text-[#9c5e6c] rounded-full px-2 py-0.5 font-semibold">Основная</span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-1">
                  {!method.isDefault && (
                    <button onClick={() => handleSetDefault(method.id)}
                      className="p-1.5 rounded-lg hover:bg-[#f4f3ef] text-[#7a7290]" title="Сделать основной">
      ...
      {showModal && <AddCardModal onClose={() => setShowModal(false)} onAdded={() => void load()} />}
    </div>
  );
}
