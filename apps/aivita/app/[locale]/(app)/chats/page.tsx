...
interface LastMessage {
  ...
  attachmentName?: string; createdAt: string;
}
interface Conv {
...
export default function PatientChatsPage() {
  ...
  // Refresh unread every 30s
  useEffect(() => {
    const t = setInterval(() => {
      fetch(`${PROXY}/conversations`)
        .then(r => r.json())
        .then(j => setConvs(j.data ?? []))
        .catch(() => {});
    }, 30000);
  ...
  return (
    <div className="max-w-[480px] mx-auto">
      ...
      <div className="sticky top-0 z-20 px-4 pt-12 pb-3"
        ...
        <h1 className="text-[22px] font-extrabold" style={{ color: '#2a2540' }}>Мои чаты</h1>
        ...
        {convs.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-sm text-[#9a96a8] mb-6">
              Найдите врача и начните переписку
            </p>
              ...
              <button key={conv.id}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <p className="font-semibold text-[#2a2540] text-sm truncate">
                      Dr. {conv.otherUser?.name ?? 'Врач'}
                    </p>
                  ...
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-[#9a96a8] truncate flex-1">
                      {previewMsg(conv.lastMessage)}
                    </p>
                    {conv.unreadCount > 0 && (
