...
// ─── Client component (no custom header — TopBar from PageShell provides it) ──
          ...
          <>
            ...
            <button
              ...
              style={{
                ...
                color: selectedSymptom ? '#fff' : C.t3, fontSize: 16, fontWeight: 700,
                cursor: selectedSymptom ? 'pointer' : 'default', transition: 'all .2s',
              }}>
                ...
                {stepData.options.map(opt => (
                  <button key={opt} onClick={() => !loading && handleAnswer(opt)} disabled={loading}
                    style={{
                      ...
                      cursor: loading ? 'default' : 'pointer', textAlign: 'left', transition: 'all .15s',
                    }}>
                    {opt}
                  </button>
                ))}
              </div>
              {loading && <div style={{ textAlign: 'center', marginTop: 16, color: C.t3, fontSize: 13 }}>⏳ Анализирую...</div>}
