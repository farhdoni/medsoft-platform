...
// Re-export the entire MentalHealthPage logic as a named client component.
// The custom header (← back + title) has been removed — PageShell provides
...
function BreathingCircle({ phase, label, duration }: { phase: number; label: string; duration: number }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 24, padding: '30px 0' }}>
      <div style={{ width: 160, height: 160, borderRadius: '50%', background: `radial-gradient(circle, ${C.purpleBg} 0%, ${C.blueBg} 100%)`, border: `3px solid ${C.purple}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', transform: `scale(${scale})`, transition: `transform ${duration}s ease-in-out`, boxShadow: `0 0 ${isHold ? '40px' : '20px'} ${C.purple}30` }}>
...
export function MentalHealthClient() {
  ...
  useEffect(() => {
    if (tab === 'mood' && moodHistory.length === 0) {
      fetch('/api/proxy/mental/mood?period=30d').then(r => r.json())
        .then((j: { data?: MoodEntry[] }) => { if (j.data) setMoodHistory(j.data); }).catch(() => {});
    }
    if (tab === 'therapist' && chatMessages.length === 0) {
      fetch('/api/proxy/mental/therapist/history').then(r => r.json())
        .then((j: { data?: ChatMessage[] }) => { if (j.data?.length) setChatMessages(j.data); }).catch(() => {});
    }
  ...
  const handleSaveMood = async () => {
    ...
    try {
      ...
      if (json.data) { setMoodSaved(true); setMoodHistory(prev => [json.data!, ...prev]); }
    } catch { /* silent */ }
    finally { setMoodLoading(false); }
  };

  const startBreathing = useCallback(() => {
    setBreathRunning(true); setBreathPhase(0); setBreathTimer(breathEx.pattern[0]);
    let phaseIdx = 0; let phaseTime = breathEx.pattern[0];
    breathIntervalRef.current = setInterval(() => {
      phaseTime -= 1; setBreathTimer(phaseTime);
      if (phaseTime <= 0) {
        phaseIdx = (phaseIdx + 1) % breathEx.pattern.length;
        if (phaseIdx === 0) setBreathCycles(c => c + 1);
        phaseTime = breathEx.pattern[phaseIdx];
        setBreathPhase(phaseIdx); setBreathTimer(phaseTime);
      }
    }, 1000);
  }, [breathEx]);
  ...
  const startMeditation = useCallback((med: typeof MEDITATIONS[0]) => {
    setSelectedMed(med); setMedRunning(true); setMedSeconds(0); setMedStep(0);
    let secs = 0;
    medIntervalRef.current = setInterval(() => {
      secs += 1; setMedSeconds(secs);
      const totalSecs = med.duration * 60;
      if (secs >= totalSecs) { if (medIntervalRef.current) clearInterval(medIntervalRef.current); setMedRunning(false); return; }
      const stepIdx = med.steps.findIndex((s, i) => { const next = med.steps[i + 1]; return secs >= s.t && (!next || secs < next.t); });
  ...
  const stopMeditation = useCallback(() => {
    ...
    setMedRunning(false); setSelectedMed(null);
  }, []);
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100%' }}>
      {/* Tabs — replaces the old tab bar that was below the custom header */}
      <div style={{ background: C.card, padding: '0 12px', borderBottom: `1px solid ${C.border}`, display: 'flex', gap: 2, overflowX: 'auto' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id as typeof tab)}
            style={{ padding: '12px 10px', border: 'none', background: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap', color: tab === t.id ? C.accent : C.t3, borderBottom: `2.5px solid ${tab === t.id ? C.accent : 'transparent'}` }}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          <>
            {!moodSaved && !todayMood ? (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: 20 }}>
                  {MOODS.map(m => (
                    <button key={m.v} onClick={() => setSelectedMood(m.v)}
                      style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '10px 8px', borderRadius: 14, border: `2px solid ${selectedMood === m.v ? m.color : C.border}`, background: selectedMood === m.v ? m.color + '20' : '#fafaf8', cursor: 'pointer', transition: 'all .15s' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 14 }}>
                      {FACTORS.map(f => (
                        <button key={f} onClick={() => setSelectedFactors(prev => prev.includes(f) ? prev.filter(x => x !== f) : [...prev, f])}
                          style={{ padding: '5px 12px', borderRadius: 20, border: `1.5px solid ${selectedFactors.includes(f) ? C.accent : C.border}`, background: selectedFactors.includes(f) ? C.accentBg : '#fafaf8', fontSize: 12, fontWeight: 600, color: selectedFactors.includes(f) ? C.accent : C.t2, cursor: 'pointer' }}>
                          {f}
                        </button>
                      ))}
                    </div>
              ...
              <div style={{ background: C.greenBg, borderRadius: 20, padding: 20, marginBottom: 16, border: `1px solid ${C.green}30`, textAlign: 'center' }}>
                <p style={{ fontSize: 32, marginBottom: 8 }}>{MOODS.find(m => m.v === (todayMood?.mood ?? selectedMood))?.emoji ?? '😊'}</p>
                <p style={{ fontSize: 15, fontWeight: 700, color: C.green, marginBottom: 4 }}>Настроение сегодня сохранено!</p>
                <p style={{ fontSize: 13, color: C.t2 }}>{MOODS.find(m => m.v === (todayMood?.mood ?? selectedMood))?.label}{(todayMood?.factors?.length ?? 0) > 0 && ` · ${todayMood?.factors.join(', ')}`}</p>
              </div>
            )}
                  ...
                  {moodHistory.slice(0, 7).map(e => {
                    ...
                    return (
                      <div key={e.id} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                        ...
                        <div style={{ flex: 1 }}><span style={{ fontSize: 12, fontWeight: 600, color: C.t2 }}>{e.date}</span>{e.note && <p style={{ fontSize: 11, color: C.t3, margin: '2px 0 0' }}>{e.note}</p>}</div>
        ...
        {tab === 'breathing' && (
          <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
                {BREATHING_EXERCISES.map(ex => (
                  <button key={ex.id} onClick={() => { setBreathEx(ex); setBreathCycles(0); }}
                    style={{ padding: '16px 18px', borderRadius: 16, border: `2px solid ${breathEx.id === ex.id ? C.purple : C.border}`, background: breathEx.id === ex.id ? C.purpleBg : C.card, textAlign: 'left', cursor: 'pointer' }}>
                    ...
                    <p style={{ fontSize: 11, color: C.t3, margin: '4px 0 0' }}>{ex.pattern.map((p, i) => `${ex.phases[i]} ${p}с`).join(' → ')}</p>
                  </button>
                ))}
              </div>
            )}
            <div style={{ background: tab === 'breathing' ? '#1a1630' : C.card, borderRadius: 24, padding: 24, border: `1px solid ${C.border}`, marginBottom: 16 }}>
              {breathRunning ? (
                <>
                  <BreathingCircle phase={breathPhase} label={breathEx.phases[breathPhase]} duration={breathEx.pattern[breathPhase]} />
                  <p style={{ textAlign: 'center', color: '#c0b8d0', fontSize: 13, marginBottom: 8 }}>Цикл {breathCycles + 1}</p>
                  <button onClick={stopBreathing} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: '#ffffff20', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>⏹ Остановить</button>
                </>
              ) : (
                <div style={{ textAlign: 'center', padding: '20px 0' }}>
                  ...
                  <button onClick={startBreathing} style={{ padding: '14px 40px', borderRadius: 14, border: 'none', background: `linear-gradient(135deg, ${C.purple}, ${C.blue})`, color: '#fff', fontSize: 16, fontWeight: 700, cursor: 'pointer' }}>▶ Начать</button>
            ...
            {!medRunning ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {MEDITATIONS.map(med => (
                  <button key={med.id} onClick={() => startMeditation(med)}
                    ...
                    <div style={{ flex: 1 }}><p style={{ fontSize: 15, fontWeight: 700, color: C.t1, margin: 0 }}>{med.label}</p><p style={{ fontSize: 12, color: C.t3, margin: '4px 0 0' }}>{med.duration} мин</p></div>
              ...
              <div style={{ background: '#1a1630', borderRadius: 24, padding: 24, minHeight: 400, display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '20px 0' }}>
                  {medPct >= 100 ? (<><p style={{ fontSize: 48 }}>🎉</p><p style={{ fontSize: 20, fontWeight: 800, color: '#fff', marginTop: 12 }}>Завершено!</p></>) : (<p style={{ fontSize: 18, fontWeight: 600, color: '#e0d8f0', lineHeight: 1.6, maxWidth: 280 }}>{selectedMed.steps[medStep]?.text}</p>)}
                </div>
                <button onClick={stopMeditation} style={{ width: '100%', padding: '14px', borderRadius: 14, border: 'none', background: '#ffffff15', color: '#c0b8d0', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12, paddingBottom: 16 }}>
              ...
              {chatMessages.map((m, i) => (
                <div key={i} style={{ display: 'flex', justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  <div style={{ maxWidth: '80%', padding: '12px 16px', borderRadius: 18, background: m.role === 'user' ? `linear-gradient(135deg, ${C.accent}, #7a3848)` : C.card, border: m.role === 'assistant' ? `1px solid ${C.border}` : 'none', color: m.role === 'user' ? '#fff' : C.t1, fontSize: 14, lineHeight: 1.5 }}>
                    {m.content}
                  </div>
                </div>
              ))}
              {chatLoading && (<div style={{ display: 'flex', justifyContent: 'flex-start' }}><div style={{ padding: '12px 16px', borderRadius: 18, background: C.card, border: `1px solid ${C.border}`, color: C.t3, fontSize: 14 }}>✍️ Печатает...</div></div>)}
              <div ref={chatEndRef} />
            </div>
            <div style={{ display: 'flex', gap: 10, padding: '12px 0 0', borderTop: `1px solid ${C.border}` }}>
              <input value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendTherapistMessage(); } }}
