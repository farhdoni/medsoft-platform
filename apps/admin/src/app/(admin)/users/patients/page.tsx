...
export default function PatientsPage() {
  ...
  useEffect(() => { resetPage(); }, [debouncedSearch, tier, status, sortOrder, resetPage]);
  ...
  const { data, isLoading } = useQuery<ListResponse>({
    queryKey: ['admin-users-patients', queryParams],
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        isLoading={isLoading}
        onRowClick={(row) => router.push(`/users/patients/${row.id}`)}
      />
    </div>
  );
}
