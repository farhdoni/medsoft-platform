...
CREATE TYPE "public"."blood_group" AS ENUM('O_minus', 'O_plus', 'A_minus', 'A_plus', 'B_minus', 'B_plus', 'AB_minus', 'AB_plus', 'unknown');--> statement-breakpoint
CREATE TYPE "public"."patient_status" AS ENUM('pending_verification', 'active', 'suspended', 'premium');--> statement-breakpoint
CREATE TYPE "public"."clinic_status" AS ENUM('pending', 'active', 'suspended');--> statement-breakpoint
CREATE TYPE "public"."clinic_type" AS ENUM('hospital', 'clinic', 'laboratory', 'pharmacy', 'diagnostic_center');--> statement-breakpoint
CREATE TYPE "public"."doctor_status" AS ENUM('pending', 'active', 'suspended', 'offline');--> statement-breakpoint
CREATE TYPE "public"."ai_intent" AS ENUM('symptom_check', 'doctor_recommendation', 'medication_info', 'lifestyle_advice', 'other');--> statement-breakpoint
CREATE TYPE "public"."ai_outcome" AS ENUM('appointment_booked', 'self_care_advised', 'sos_triggered', 'no_action', 'abandoned');--> statement-breakpoint
CREATE TYPE "public"."payment_provider" AS ENUM('click', 'payme', 'uzcard', 'humo', 'internal_deposit', 'manual');--> statement-breakpoint
CREATE TYPE "public"."transaction_status" AS ENUM('pending', 'completed', 'failed', 'refunded', 'cancelled');--> statement-breakpoint
CREATE TYPE "public"."transaction_type" AS ENUM('deposit_topup', 'appointment_payment', 'refund', 'withdrawal', 'commission_to_clinic', 'bonus');--> statement-breakpoint
CREATE TYPE "public"."appointment_status" AS ENUM('scheduled', 'confirmed', 'in_progress', 'completed', 'cancelled_by_patient', 'cancelled_by_doctor', 'no_show');--> statement-breakpoint
CREATE TYPE "public"."appointment_type" AS ENUM('telemedicine_video', 'telemedicine_chat', 'offline_clinic', 'home_visit');--> statement-breakpoint
CREATE TYPE "public"."sos_status" AS ENUM('triggered', 'operator_assigned', 'brigade_dispatched', 'in_progress', 'resolved', 'false_alarm', 'cancelled');--> statement-breakpoint
...
CREATE TABLE "sos_calls" (
	...
	"brigad_dispatched_at" timestamp with time zone,
	"brigad_arrived_at" timestamp with time zone,
...
ALTER TABLE "ai_logs" ADD CONSTRAINT "ai_logs_recommended_doctor_id_doctors_id_fk" FOREIGN KEY ("recommended_doctor_id") REFERENCES "public"."doctors"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "transactions" ADD CONSTRAINT "transactions_initiated_by_admin_id_admin_users_id_fk" FOREIGN KEY ("initiated_by_admin_id") REFERENCES "public"."admin_users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "sos_calls" ADD CONSTRAINT "sos_calls_patient_id_patients_id_fk" FOREIGN KEY ("patient_id") REFERENCES "public"."patients"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "sos_calls" ADD CONSTRAINT "sos_calls_assigned_admin_id_admin_users_id_fk" FOREIGN KEY ("assigned_admin_id") REFERENCES "public"."admin_users"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
