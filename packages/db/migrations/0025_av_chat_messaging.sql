...
--
-- ПРЕДУПРЕЖДЕНИЕ (задним числом, добавлено при 0042). Эта миграция была
-- отредактирована ПОСЛЕ применения: коммит 1167fc7 выпустил её с наивными
-- `timestamp`-колонками, коммит c979aba переписал их здесь же на timestamptz
-- — без единого ALTER. Поскольку ниже везде CREATE TABLE IF NOT EXISTS, любая
-- база, поднятая между этими двумя коммитами, тихо осталась наивной: у неё
-- этот файл уже был выполнен в старой форме, и молчаливый no-op ничего не
-- исправил. Починка — 0042_avchat_timestamptz_catchup.sql, идемпотентная,
-- сравнивает объявленный тип миграции с фактическим типом в базе.
--
-- ПРАВИТЬ УЖЕ ПРИМЕНЁННЫЕ МИГРАЦИИ ЗАПРЕЩЕНО. Исправление всегда — новый
-- файл; см. 0042 как образец.
-- ─── 1. Enums ─────────────────────────────────────────────────────────────────
