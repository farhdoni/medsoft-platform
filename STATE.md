...
Обновлено: 2026-09-09 · Ветка: `main` · Правило владельца: один агент — одна рабочая копия

## 🔒 Правило: одна рабочая копия (`C:\MedSoft`) — один агент

Записано владельцем 2026-09-09 после инцидента: параллельная сессия переключила общий чекаут
`C:\MedSoft` на свою ветку (`feature/medsoft-analytics`), и следующий коммит ДРУГОЙ, ничего не
подозревающей сессии лёг поверх этого чужого незапушенного коммита вместо своей ветки. Разобрано без
потерь (коммит перенесён на верную ветку, дубликат убран, чужая ветка не тронута), но по факту, а не
по процессу — до этого правила такая коллизия была возможна в принципе. См. также аналогичную запись
в `marketing-engine-php/CONTEXT.md` (раздел «Регламент для агентов»).

**Правила:**
1. В одной рабочей копии (`C:\MedSoft`) работает один агент. Второй агент — отдельная копия через
   `git worktree add ../<имя> <ветка>`, не переключение веток в общем чекауте.
2. Перед каждым коммитом — `git branch --show-current`. Коммитить только если ветка та, что ожидалась
   для этой сессии; если нет — остановиться, не коммитить вслепую.
3. Перед каждым пушем — то же самое: проверить текущую ветку перед `git push`.
